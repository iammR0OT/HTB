...
Visual was ranked as a medium Windows Server 2019 machine involves abusing the **PreBuildEvent** and **PostBuildEvent** features of Visualstudio, which are vulnerable to running any command inside the powershell, through command injection we got shell as Enox user. For root, we discover that the **htdocs** folder has permissions that user enox can create and execute code here, so we will host our cmd.php file here and get the **nt local/service** account through the web browser, and then have to run **FullPowers.exe** to get our permissions back. Then we have **sImpersination** privileges on our user, then we will run **SharpEFSPotato.exe**, which impersonates our user privilege with the privileges of the admin account, and finally we will get root on the machine.
...
```bash
$ nmap -p80 -sC -sV -A -T4 10.10.10.175 -oN scan.txt
```
...
Our project has been successfully created. Let's navigate to the project directory and run `dotnet new sln`. After that, add the `.csproj` file to the solution using `dotnet sln add MyApp.csproj`.

To obtain a shell, we need to insert our reverse shell code into the `MyApp.csproj` file between `<PreBuildEvent>` tags. You can use the website **Revshells.com** for this purpose. We will use the PowerShell base64 shell.
